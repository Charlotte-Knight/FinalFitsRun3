...
def robust_fit(extroopdf, pdf, datahist, fit_ranges_str, n_fits=8, recursive=True, max_n_fits=1024):
  nlls = []
  params_vals = []
  ...
  for i in range(n_fits):
    ...
    params_vals.append([var.getVal() for var in pdf.params])

  best_params_vals = params_vals[np.argmin(nlls)]
  for i, val in enumerate(best_params_vals):
    pdf.params[i].setVal(val)
  ...
  if nll_diff > max_diff:
    ...
    if recursive and n_fits < max_n_fits:
      ...
      robust_fit(extroopdf, pdf, datahist, fit_ranges_str, n_fits_more, max_n_fits=max_n_fits)
    else:
...
def fit(pdf, datahist, fit_ranges=(), method="robust"):
  ...
  if method == "robust":
    robust_fit(extroopdf, pdf, datahist, fit_ranges_str)
  else:
    if method == "randomize":
      pdf.randomize_params()
    extroopdf.fitTo(datahist, Range=fit_ranges_str, PrintLevel=-1, SumW2Error=True)
...
def main(in_file, out_file, pdf_name="Gaussian", order=1, fit_ranges=(), #
  ...
  x = ROOT.RooRealVar("x", "x", 100, 300)
  data = f.Get("h_mjj_sig")
  ...
  fit(pdf, datahist, fit_ranges=fit_ranges, method="randomize")
  print(pdf)

  if plot_savepath is not None:
    xlim = (x.getMin(), x.getMax()) if plot_range == () else plot_range
    plotting.plotFit(datahist, x, pdf.roopdf, pdf.params, plot_savepath, xlim=xlim)
    plotting.plotFitRoot(datahist, x, pdf.roopdf, pdf.params, plot_savepath, xlim=xlim)

...
def robust_fit(pdf, datahist, fit_ranges_str, n_fits=8, recursive=True,
  ...
  for i in range(n_fits):
    ...
    r = pdf.extroopdf.fitTo(datahist, Range=fit_ranges_str, PrintLevel=-1, Save=True, SumW2Error=True)
    fit_results.append(r)

  max_diff = 0.01
  nlls = [r.minNll() for r in fit_results]
  ...
  if nll_diff > max_diff:
    ...
    if recursive and n_fits < max_n_fits:
      ...
      return robust_fit(pdf, datahist, fit_ranges_str, n_fits_more,
  ...
  return fit_results[np.argmin(nlls)]
...
def fit(pdf, datahist, fit_ranges=(), method="robust", seed=None):
  ...
  if method == "robust":
    r = robust_fit(pdf, datahist, fit_ranges_str, seed=seed)
  else:
    if method == "randomize":
      pdf.randomize_params(seed)
    r = pdf.extroopdf.fitTo(datahist, Range=fit_ranges_str, PrintLevel=-1, SumW2Error=True, Save=True)

  pdf.fit_result = r
  pdf.check_bounds()
  
  two_nll = 2*pdf.roopdf.createNLL(datahist, Range=fit_ranges_str, Offset="bin").getVal()
  
  n_free_params = r.floatParsFinal().getSize()
  fit_dof = int(utils.getNBinsFitted(pdf.x, fit_ranges) - n_free_params)
  gof_pval = ROOT.TMath.Prob(two_nll, fit_dof)
  return {"twoNLL": two_nll, "gof_pval": gof_pval}
...
def main(in_file, out_file, pdf_name="Gaussian", order=1, fit_ranges=(), #
  ...
  if plot_savepath is not None:
    ...
    plotting.plotFit(datahist, pdf, x, plot_savepath, xlim=xlim)
    plotting.plotFitRoot(datahist, pdf, x, plot_savepath, xlim=xlim)
